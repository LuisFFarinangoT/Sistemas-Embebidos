...
void  espectron( int col, int intentos ) //neurona para la matriz de prueba
...
*/
{
  ...
  for (int t = 0; t < intentos;t++) {
    ...
    { if (n[s] >= 0)//comparacion del valor de la suma ponderada
      ...
      if (neuro[s] == int(entre[s][2]))//se compara el valor obtenido con el esperado
      {
        cont++;//contador para el número de aciertos
      }
      else {//condici+on para el cambio de peso
        ps1[s] = ps1[s] + (0.5 * ((1 - n[s]) * entre[s][0])); //formula cambio de peso
        ps2[s] = ps2[s] + (0.5 * ((2 - n[s]) * entre[s][1]));//formula cambio de peso
  ...
  //    }
  //    else {//condici+on para el cambio de peso
  //      ps1[k] = ps1[k] + (0.5 * ((1- n[k]) * entre[k][0]));//formula cambio de peso
  //      ps2[k] = ps2[k] + (0.5 * ((2 - n[k]) * entre[k][1]));//formula cambio de peso
  //      entrena[k][0] = ps1[k];//se llena una matriz para usarse en la matriz de datos totales
  //      entrena[k][1] = ps2[k];//se llena una matriz para usarse en la matriz de datos totales
  //    }
  //    delay(10);
...
void loop() {
  ...
  //    }
  //    predo = (predo * 100) / 380;
  //    Serial.println("El rendimiento final es de" + predo); //impresion de contador
  //  }
